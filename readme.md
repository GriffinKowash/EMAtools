...
When in doubt, refer to function-specific documention in the source code at https://github.com/GriffinKowash/EMAtools/tree/main/src/ema. Please notify Griffin at griffin.kowash@ema3d.com if the behavior of a function contradicts the documentation.

## Probes module

### Loading point probe results

Any probe results that are formatted with a single time step per line can be loaded as follows:
...
```
Note that for time series results, the first index of the ```results``` array will always correspond to the time steps.


### Loading box/distributed probe results

Loading box and distributed probe results requires a different function:

```
t, data = ema.load_box_probe('path/to/box_probe.dat')
```

The `t` array is a 1D NumPy array of the measurement timesteps, while `data` is a 3D array containing the measurement data.
By default, `data` has dimensions of [time, component, sample]. The "component" index corresponds to the X, Y, and Z components of the field measurement, while "sample" refers to the individual sample points defined over the probed region. For example, to obtain the Y component at the fifth sample point for every timestep:
  
```
ey_5 = data[:, 1, 4]
```

The default dimensions of [timestep, X/Y/Z component, sample point] can be changed by providing the optional `last_index` argument. For example, the following will return a `data` array with dimensions [time, sample, component]:
  
```
t, data = ema.load_box_probe('path/to/box_probe.dat', last_index='component')
```

Alternatively, providing `None` for this argument will return a flattened 2D data array of shape [time, samples*components], with the X, Y, and Z measurements from each sample point stacked sequentially along the second axis.
  ...
  Note that probe results generated by older versions of EMA3D may fail to load due to a difference in file format. The oldest version known to be supported is currently 2023R2.
...
`ema.rfft` computes the FFT along axis 0 by default. If the input array `x` has multiple dimensions, the `axis` argument can be used to apply the FFT along a different axis. For example, if axis 0 of `x` corresponds to a collection of probes and axis 1 corresponds to time, the FFT can be computed as:

```
f, xfft = ema.rfft(t, x, axis=1)
```

If the first and last values of a time series measurement are not equal, the discontinuity will introduce erroneous spectral content to the FFT. Window functions are often used to address this problem. A Hann window can be easily applied using the `window` keyword argument:
...
Time series data (timesteps and measurements) can conveniently be trimmed to a desired time window. For example, to truncate a dataset to an end time of 5ns:
...
The `File.printlines` method prints a line or range of lines to the console with readable formatting, which is useful when testing a script's functionality. For example, executing `file.printlines(10, 15)` will print something like the following to the console:
...
Alternatively, a list of line numbers can be provided. For example, `emin.printlines([1, 14, 25])` will print the following:
...
Any change to a simulation file typically begins with locating a particular string of text. The base function for this task is `File.find_all`, which returns a list of all indices at which the provided text string is present in the file. For example, to search for every instance of `!NEW PROBE FILE NAME`:
...
The `File.replace` method conveniently combines the functionality of `File.remove` and `File.insert` to help the user replace one or more lines in the file with new text. To replace the line at index 15 with a comment:

```
file.replace(15, '* Replaced index 15 with one comment')
```

The user can provide an endpoint-inclusive range of indices to replace:

```
file.replace((15, 20), '* Replaced indices 15 to 20 with one comment')
```

The selected range can be replaced with multiple lines by providing a list of strings:
...
The `Emin.modify_isotropic_material` method allows the user to easily modify all instances of a given isotropic material. For example, the conductivity of a material named "CFRP" can be set to 300 S/m as follows:
